...
function initMainMap() {
    ...
    // Create map centered on all properties (zoom 4 = USA fills map)
    mainMap = L.map('main-map').setView([centerLat, centerLon], 4);
    ...
    // Fit bounds but keep zoom level 4 minimum (USA fills map)
    if (mainMapMarkers.length > 0) {
        const group = L.featureGroup(mainMapMarkers);
        mainMap.fitBounds(group.getBounds().pad(0.01), { maxZoom: 4 });
    }
}
